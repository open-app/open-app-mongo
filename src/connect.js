...
function connect (sbot, messageType, opts) {
  ...
  return function (Collection) {
    ...
    var source = many(messageType.map(function (type) {
      var gte = opts.gte ||  null // Date.now()
    ...
    var worker = function (message, cb) {
      ...
      var insert = function (content) {
        if (content == null || typeof(content) !== 'object') return cb() // ignore if content is null of message is private
        Collection.insert(Object.assign({ _id: message.key }, content), cb)
      }
